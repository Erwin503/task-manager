...
const createTest = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ data: test, message: "Test and answers created successfully" });
  } catch (error) {
    res.status(500).json({ data: null, message: error.message });
  }
};
  ...
  try {
    ...
    if (testAnswer) {
      ...
      if (isCorrect) {
        ...
        const correctAnswer = await TestAnswer.findOne({
          where: { testId: req.params.testId, isCorrect: true },
        });
        res.json({
          data: { isCorrect, correctAnswer: correctAnswer.text },
...
const getTestsByUser = async (req, res) => {
  try {
    ...
    res.json({ data: tests, message: "Tests for user retrieved successfully" });
  } catch (error) {
    res.status(500).json({ data: null, message: error.message });
  }
};
